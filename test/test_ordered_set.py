...
@pytest.mark.parametrize("other", [[2, 3, 3], (2, 3, 3)])
def test_symmetric_difference_update_duplicate_input(other):
    oset = OrderedSet([1, 2])
    oset.symmetric_difference_update(other)
    assert list(oset) == [1, 3]
    assert len(oset) == 2
    assert oset.index(3) == 1
    assert oset[oset.index(3)] == 3
